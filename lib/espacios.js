...
const geometria = require('./geometriaDelPlano.js');
...
/* El paso de la rejilla. Lo sabe `geometriaDelPlano` porque el dibujo tiene
   que estar de acuerdo con el generador: si cada uno tuviera el suyo, cambiar
   uno descolocaría los planos ya guardados. */
const PASO_REJILLA = geometria.PASO;
...
function generarUnidades({ filas = 1, porFila = 1, tipo = 'silla', capacidad = 1,
                           ...
                           /* 'rejilla' es lo de siempre y sigue siendo el valor
                              por defecto: nada de lo ya generado cambia. */
                           forma = 'rejilla', curva = {} } = {}) {
  ...
  /* En abanico las sillas se colocan sobre arcos concéntricos que miran al
     escenario, que es como están en un teatro de verdad. Las coordenadas se
     calculan aquí, una vez, y a partir de ahí cada silla es un punto como
     cualquier otro: se puede mover a mano, ponerle precio y venderla.

     Se pide `porFila` fijo a propósito aunque el generador sepa calcular
     cuántas caben: quien numera butacas necesita que la fila C tenga las
     mismas que la D, y un plano donde cada fila tiene un número distinto de
     sillas es imposible de comprobar contra el recinto real. */
  const enAbanico = forma === 'abanico'
    ? geometria.sillasEnAbanico({
        filas: nf, porFila: nc,
        cx: Number(curva.cx) || 0, cy: Number(curva.cy) || 0,
        radioPrimera: Number(curva.radioPrimera) || 200,
        separacion: Number(curva.separacion) || geometria.PASO,
        abertura: Number(curva.abertura) || 90,
        mirandoA: curva.mirandoA || 'arriba',
      })
    : null;
  ...
  for (let f = 0; f < nf; f++) {
    for (let c = 0; c < nc; c++) {
      ...
      unidades.push({
        ...
        /* Dónde queda. El editor la mueve después; esto es para que algo se vea
           desde el primer momento en vez de un montón de puntos en el origen.
           En abanico viene con `rot`, los grados que la ponen de cara al
           escenario: sin eso las sillas de los extremos salen torcidas
           respecto a su fila, que es lo que delata un plano generado. */
        /* Sólo x, y y rot: `sillasEnAbanico` devuelve además la fila y el
           número que usó para colocar, y eso ya vive en el nombre y en `orden`.
           Guardarlo otra vez dentro de la geometría sería el mismo dato en dos
           sitios, que es como empiezan a discrepar. */
        geometria: enAbanico
          ? { x: enAbanico[f][c].x, y: enAbanico[f][c].y, rot: enAbanico[f][c].rot }
          : { x: c * PASO_REJILLA, y: f * PASO_REJILLA },
        orden: f * 1000 + numero,
      });
    }
  }
