...
const BLOQUES = {
  ...
  /* Lo que el evento HACE. Faltaban los dos: el catálogo dejaba poner
     patrocinadores y testimonios, pero no el programa ni el torneo. Sin esto en
     el contrato, un asistente que arme la página por MCP no puede ponerlos —y
     el servidor rechazaría el bloque aunque el panel lo pintara. */
  agenda: {
    label: 'Programa / agenda', categoria: 'custom',
    campos: {
      titulo: { tipo: 'texto', max: 200 },
      subtitulo: { tipo: 'texto', max: 500 },
      /* Cuántas actividades caben antes de que esto deje de ser una portada. */
      limite: { tipo: 'numero', min: 1, max: 24 },
    },
  },
  torneos: {
    label: 'Torneos', categoria: 'custom',
    campos: { titulo: { tipo: 'texto', max: 200 }, subtitulo: { tipo: 'texto', max: 500 } },
  },
  /* ── Código propio ──────────────────────────────────────────────────────
   *
   * El único bloque que acepta HTML libre, y por eso el único que necesita esta
   * explicación:
   *
   * **No se pinta en la página.** Se pinta dentro de un `iframe` con `sandbox`
   * y SIN `allow-same-origin`, que es lo que lo cambia todo: ahí dentro el
   * código corre en un origen opaco —no puede leer las cookies del visitante, ni
   * el token de sesión del organizador, ni tocar la página de alrededor, ni
   * llamar a la API en nombre de nadie—.
   *
   * Sin ese aislamiento, un `<script>` aquí correría con el origen del evento y
   * podría robar la sesión de quien lo está editando. Con él, lo peor que puede
   * hacer es estropear su propio recuadro.
   *
   * Por eso el servidor **no sanea** el HTML: sanear invita a creer que el
   * contenido es seguro y a bajar la guardia el día que alguien quite el
   * sandbox. Lo que se guarda es texto; lo que lo hace inofensivo es dónde se
   * pinta, y eso se comprueba en el frontend con una prueba. */
  codigo: {
    label: 'Código propio', categoria: 'custom',
    campos: {
      titulo: { tipo: 'texto', max: 200 },
      /* 100 KB: cabe un widget de verdad y no cabe una página entera pegada,
         que es como `page_json` se convierte en algo que no se puede cargar. */
      html: { tipo: 'texto', max: 100000 },
      alto: { tipo: 'numero', min: 80, max: 2000 },
    },
  },
  mapa_evento: {
