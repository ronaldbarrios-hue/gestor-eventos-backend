...
/* Quema el token y cierra la fila de esa persona. Devuelve `true` sólo a quien
   ...
   ── Por qué el token se queda escrito ───────────────────────────────────
   Antes esto ponía `oferta_token` a null «para quemarlo». No hace falta: el
   enlace ya no vale porque `validarOferta` exige `estado === 'contacted'`, y
   borrarlo hacía imposible DEVOLVER la oferta si la boleta no llegaba a
   crearse — se llama antes de emitir, así que un error al insertar dejaba a
   esa persona sin sitio y sin boleta, que es lo peor de los dos mundos. */
...
/* El paso atrás: la oferta se tomó y la boleta no salió.
   Sólo devuelve la que devolvió ESTA petición (`estado === 'purchased'` y sin
   compra), y sólo si el plazo sigue vivo: pasado el plazo el sitio ya es del
   siguiente de la fila y resucitarlo sería quitárselo a él. */
async function devolverOferta(waitlistId) {
  if (!waitlistId) return false;
  try {
    const { data } = await supabase.from('event_waitlist').update({
      estado      : 'contacted',
      purchased_at: null,
    })
      .eq('id', waitlistId)
    ...
    console.warn('[waitlist] no se pudo devolver la oferta:', e.message);
    return false;
  }
}
