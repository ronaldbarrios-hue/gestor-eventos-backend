...
const { baseFrontend, frontendDeclarado } = require('./frontend.js');
...
const TIPOS = [
  ...
  {
    /* La rueda tiene DOS momentos y hasta ahora se contaban los dos con el
       mismo correo. En modo «solicitud» la cita nace pendiente de que el
       equipo la apruebe, y aun así llegaba «tu cita quedó confirmada»: la
       persona se presentaba a una hora que nadie le había dado y no había
       nadie esperándola. */
    id: 'cita_pedida',
    label: 'Cita solicitada',
    descripcion: 'Rueda de negocios: acusa recibo de una cita que aún tiene que aprobar el equipo.',
    automatico: true,
    variables: ['nombre', 'evento', 'fecha', 'hora', 'lugar', 'organizador', 'enlace'],
    defaults: {
      asunto:      'Recibimos tu solicitud de cita en {{evento}}',
      encabezado:  'Solicitud recibida',
      cuerpo:      'Hola {{nombre}}, pediste una cita en {{evento}} para el {{fecha}} a las {{hora}}.\n\nTodavía no está confirmada: el equipo la revisa y te avisamos en cuanto responda. No hace falta que hagas nada.',
  ...
  {
    /* El aviso de que la reunión es AHORA.
     *
     * En una rueda, la reunión a la que nadie se presenta es el peor
     * resultado: la mesa esperó, la casilla figuraba ocupada —así que nadie
     * más pudo pedirla— y no salió nada de ella. El correo de confirmación es
     * de hace tres semanas y no le recuerda a nadie que su cita es a las
     * 10:15 en la mesa 12.
     *
     * Lleva el sitio en el asunto a propósito: mucha gente lo lee de refilón
     * en la notificación del móvil, de pie y con prisa, sin llegar a abrirlo. */
    id: 'cita_recordatorio',
    label: 'Tu cita es dentro de poco',
    descripcion: 'Rueda de negocios: recuerda una cita confirmada poco antes de su hora.',
    automatico: true,
    variables: ['nombre', 'evento', 'fecha', 'hora', 'lugar', 'organizador', 'enlace'],
    defaults: {
      asunto:      'Tu cita de {{hora}} en {{lugar}}',
      encabezado:  'Tu próxima cita',
      cuerpo:      'Hola {{nombre}}, te recordamos tu cita de la rueda de negocios de {{evento}}: hoy a las {{hora}}, en {{lugar}}.\n\nSi no vas a poder ir, cancélala desde tus citas para que ese espacio quede libre para otra persona.',
      boton_texto: 'Ver mis citas',
    },
  },
  {
...
/* `cabeceraUrl` y `pieUrl` son el diseño del organizador: sube su plantilla
   partida en dos —lo de arriba y lo de abajo— y el texto y el QR van en medio,
   sobre el color que elija (`fondoCentro`).

   Por qué partida y no una imagen de fondo detrás de todo, que es lo que se
   pide siempre: una `background-image` bajo el texto se cae en Outlook de
   escritorio, Gmail la recorta en móvil, y un QR encima de una foto pierde
   lectura justo el día que más importa. Cabecera y pie son dos `<img>` de toda
   la vida: se ven igual en todos los clientes y el centro sigue siendo texto
   de verdad —seleccionable, traducible y legible para un lector de pantalla. */
function cascaron({ marca, encabezado, cuerpoHtml, extraHtml, botonTexto, botonUrl, coverUrl, piePropio, calendarioUrl, cabeceraEsDiseno = false, pieUrl = null, fondoCentro = null }) {
  ...
  /* Una cabecera DISEÑADA se muestra entera; la portada del evento se recorta.
     Recortar el diseño de alguien a 200 px le corta el logo por la mitad, que
     es exactamente lo que hace que el correo parezca roto. */
  const hero = /^https?:\/\//i.test(String(coverUrl || ''))
    ? `<tr><td style="padding:0;">
         <img src="${esc(coverUrl)}" alt="" width="560"
              style="display:block;width:100%;${cabeceraEsDiseno ? 'height:auto;' : 'max-height:200px;object-fit:cover;'}" />
       </td></tr>`
    : '';

  const pieImagen = /^https?:\/\//i.test(String(pieUrl || ''))
    ? `<tr><td style="padding:0;">
         <img src="${esc(pieUrl)}" alt="" width="560" style="display:block;width:100%;height:auto;" />
       </td></tr>`
    : '';

  /* El centro. Si el organizador eligió color, manda el suyo. */
  const superficie = hexValido(fondoCentro, marca.superficie);
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${marca.bg};padding:28px 14px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0"
             style="max-width:560px;width:100%;background:${superficie};border:1px solid ${marca.filete};border-radius:16px;overflow:hidden;">
        ...
        ${pieImagen}
        <tr><td style="padding:22px 26px 24px;${pieImagen ? '' : `border-top:1px solid ${marca.filete};`}">
...
function renderEmail({ tipo, plantilla = {}, evento = {}, ctx = {} }) {
  ...
  const html = cascaron({
    ...
    /* Si la imagen la puso la plantilla es el diseño del organizador y se
       muestra entera; la portada del evento sí se recorta. */
    cabeceraEsDiseno: Boolean(plantilla.imagen),
    pieUrl: plantilla.pie_imagen || null,
    fondoCentro: plantilla.fondo || null,
...
async function enviarEmailEvento({ evento: eventoEntrada, tipo, to, ctx = {}, registrar = true,
                                   directo = false, prioridad = 0, plantilla: plantillaFija = null }) {
  ...
  if (!directo && cola.activa()) {
    /* Si el mensaje lo escribió alguien en el momento (un recordatorio suelto,
       una campaña sin guardar), la plantilla viaja DENTRO del ctx encolado.
       Sin esto, la fila se enviaría más tarde con el texto por defecto —
       «Escribe aquí tu mensaje»— y nadie se enteraría hasta leer su bandeja. */
    const ctxCola = plantillaFija ? { ...ctx, _plantilla: plantillaFija } : ctx;
    const enc = await cola.encolar({ evento_id: evento.id, tipo, to: destino, ctx: ctxCola, prioridad });
  ...
  let asuntoFinal = '';
  try {
    /* Una plantilla puesta a mano gana sobre la guardada: es texto que alguien
       acaba de escribir para este envío concreto, no la plantilla del evento. */
    const { plantilla, origen } = plantillaFija
      ? { plantilla: plantillaFija, origen: 'directa' }
      : await plantillaDe(evento, tipo);
    ...
    /* Todos los buzones del organizador llegaron a su cupo de esta hora.

       Esto NO es un fallo y no puede tratarse como tal: si se marcara como
       error, el reintento lo volvería a intentar y acabaría forzando el tope,
       que es lo que bloquea la cuenta de correo durante días. Y tampoco puede
       caer al remitente de la plataforma: quien conectó sus buzones espera que
       salga de ahí.

       Se devuelve «espera» y la cola lo reintenta cuando el contador de la
       hora se haya vaciado. */
    if (buzonPropio?.agotados) {
      resultado = { ok: false, motivo: 'cupo_agotado', reintentar: true, origen };
    } else {
      ...
      resultado = envio?.ok
        ? { ok: true, origen, asunto, smtpId: buzonPropio?.id || null }
        : { ok: false, motivo: envio?.skipped || envio?.error || 'error', origen, smtpId: buzonPropio?.id || null };
    }
  } catch (e) {
  ...
  /* Un envío que espera por cupo no se registra: no salió ni falló, y anotarlo
     falsearía el contador que decide el relevo — cada espera restaría cupo al
     buzón como si hubiera mandado algo. */
  if (registrar && resultado.motivo !== 'cupo_agotado') {
    ...
    try {
      await supabase.from('evento_email_envios').insert({
        ...
        /* Por cuál buzón salió. Es lo que hace contable el cupo de cada uno, y
           de paso deja comparar después cuál entregó mejor. */
        smtp_id: resultado.smtpId || null,
...
/* Diagnóstico del proveedor, para el botón "Probar conexión" del panel.
   ...
   `smtp2` es el segundo buzón de plataforma (pensado para Hostinger u otro
   proveedor, alternando con el de CPANEL_SMTP_*). Con los dos puestos el
   `proveedor` pasa a 'smtp_alternado' — con sólo uno, se mantiene
   'cpanel_smtp' como siempre, para no romper a quien ya lea ese valor. */
function diagnosticoProveedor() {
  ...
  const buzonesActivos = [cpanel, smtp2].filter(Boolean).length;
  const proveedor = buzonesActivos === 2 ? 'smtp_alternado'
    : cpanel ? 'cpanel_smtp'
    : smtp2 ? 'smtp2'
    : gmail ? 'gmail_oauth'
    : resend ? 'resend'
  ...
  return {
    ...
    proveedor,
    buzones_activos: buzonesActivos,
    candidatos: { cpanel, smtp2, gmail, resend },
    remitente: process.env.EMAIL_FROM
      || (cpanel ? process.env.CPANEL_SMTP_USER : smtp2 ? process.env.CPANEL_SMTP_USER2 : process.env.GMAIL_USER)
      || null,
    ...
    /* Lo que se usará de verdad, y si viene declarado o es el de fábrica: el
       panel avisa con eso, y un valor de fábrica que «parece puesto» es
       justamente lo que hace que nadie mire. */
    frontend_url: frontendDeclarado() ? baseFrontend() : null,
    aviso: proveedor
      ? null
      : 'No hay proveedor de correo configurado: los envíos se descartan en silencio. Rellena CPANEL_SMTP_* (y CPANEL_SMTP_*2 para un segundo buzón alternando), el OAuth de Gmail o RESEND_API_KEY.',
  };
}
...
async function enviarDesdeLaCola(fila) {
  /* `_plantilla` lo metió el encolado cuando el mensaje era texto escrito a
     mano. Se saca del ctx para que no acabe entre las variables del render. */
  const { _plantilla: plantilla = null, ...ctx } = fila.ctx || {};
