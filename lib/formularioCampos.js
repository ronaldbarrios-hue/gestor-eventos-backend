...
/* Las columnas de una pregunta. `grupo` y `ayuda` los añade la 0055;
   `session_id`, la 0059. */
const COLUMNAS_CAMPO = 'id, tipo, etiqueta, opciones, requerido, orden, ticket_type_id, grupo, ayuda, buscable, visible_si, max_caracteres, max_palabras';

/* `sensible` (0115) va APARTE y no dentro de la lista de arriba.
 *
 * `COLUMNAS_CAMPO` se usa en 34 consultas. Meterle una columna nueva las rompe
 * TODAS en el hueco entre desplegar el código y aplicar la migración — y no con
 * un error claro, sino con un formulario que deja de cargar en media
 * plataforma. Ya se ha pagado esa factura en este repo.
 *
 * Sólo dos sitios necesitan saber si un campo es sensible: el editor del
 * formulario y la ruta que autoriza subir un archivo. Los dos piden esta lista
 * y saben caerse a la de siempre si la columna todavía no existe. */
const COLUMNAS_CAMPO_CON_SENSIBLE = `${'id, tipo, etiqueta, opciones, requerido, orden, ticket_type_id, grupo, ayuda, buscable, visible_si, max_caracteres, max_palabras'}, sensible`;
