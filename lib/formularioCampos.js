...
const TIPOS_CAMPO = [
  ...
  /* Un archivo cualquiera: PDF, Word, una presentación. Lo que hasta ahora se
     pedía por fuera —por correo, por WhatsApp— y se perdía: nadie sabía quién
     lo mandó ni si llegó. Si el campo se marca `sensible`, el archivo va al
     bucket privado y sólo se abre con enlace firmado. Ver
     `lib/archivoDeFormulario.js`. */
  { id: 'archivo',   label: 'Archivo adjunto (PDF, Word, presentación)', valor: 'texto' },
];
...
const COLUMNAS_PLANTILLA = [
  ...
  /* Los limites tambien por hoja: un formulario de treinta preguntas se monta
     importando, y si el limite solo se pudiera poner a mano habria que abrir
     las treinta despues de importar — o sea que en la practica no se pondria. */
  { id: 'max_caracteres', titulo: 'Max caracteres', obligatoria: false, ayuda: 'Tope de caracteres de la respuesta. Vacio = sin tope. Solo en texto corto y parrafo.' },
  { id: 'max_palabras',   titulo: 'Max palabras',   obligatoria: false, ayuda: 'Tope de palabras de la respuesta. Vacio = sin tope. Solo en texto corto y parrafo.' },
];
...
const PLANTILLA = {
  ...
  ejemplo: [
    ...
    /* La fila que ensena para que sirven las dos columnas nuevas: el ejemplo
       es el sitio donde de verdad se aprende a llenar la hoja. */
    ['8', 'Tu propuesta en pocas palabras', 'Texto corto',    'Texto',     '',                                 'No', '',                'Se breve, va en el gafete', '100', '10'],
  ],
};
...
function validarRespuesta(campo, valor) {
  ...
  const pasado = limiteDeTexto(campo, valor);
 ...
 *
 * Un formulario pide «tu propuesta en maximo 10 palabras» y hasta ahora eso
 * vivia en el enunciado sin que nadie lo hiciera cumplir: llegaban parrafos, y
 * el recorte lo acababa haciendo una persona a mano al leerlas o imprimirlas.
 *
 * Los dos limites son distintos a proposito: «100 caracteres» es espacio —cabe
 * en la etiqueta, cabe en la columna— y «10 palabras» es forma —se breve—.
 * Convertir uno en otro obliga a adivinar cuanto mide una palabra.
 *
 * Solo en texto y parrafo. En un correo o un documento el limite ya lo pone su
 * propia verificacion, y en una seleccion lo ponen las opciones.
 */
const TIPOS_CON_LIMITE = new Set(['texto', 'parrafo']);

/* La misma regla que cuenta el navegador mientras se escribe. Si las dos
   contaran distinto, el contador diria 10 y el servidor 11 — y el formulario
   se volveria imposible de enviar sin decir por que.

   Cuenta separando por espacios: los saltos de linea y los espacios dobles no
   suman, y un guion no parte la palabra («veinticuatro-siete» es una). */
function contarPalabras(texto) {
  return String(texto ?? '').trim().split(/\s+/).filter(Boolean).length;
}

function limiteDeTexto(campo, valor) {
  if (!TIPOS_CON_LIMITE.has(campo.tipo)) return null;
  const texto = String(valor ?? '');

  const maxC = Number(campo.max_caracteres) || 0;
  if (maxC > 0 && texto.length > maxC) {
    /* Se dice por cuanto se paso: «te sobran 14» es accionable, «demasiado
       largo» obliga a contar a ojo y borrar hasta que deje de quejarse. */
    return `"${campo.etiqueta}": máximo ${maxC} caracteres. Llevas ${texto.length}, te sobran ${texto.length - maxC}.`;
  }
  ...
  if (maxP > 0) {
    ...
    if (palabras > maxP) {
      return `"${campo.etiqueta}": máximo ${maxP} palabras. Llevas ${palabras}, te sobran ${palabras - maxP}.`;
    }
  }
  return null;
}
...
/* ── Preguntas condicionales ──────────────────────────────────────────────
   ...
   `visible_si = { campo, op, valor }` — ver la migración 0084. Un solo
   antecedente: la pregunta que la gente hace de verdad es «esta depende de
   aquella», y con «y/o» anidados haría falta un editor de reglas.

   Esto vive en el SERVIDOR y no sólo en el navegador porque el navegador
   puede mentir sobre qué se enseñó: si la exigencia de los obligatorios se
   decidiera allí, bastaría con no pintar un campo para saltárselo. */

function cumpleCondicion(cond, respuestas) {
  ...
  switch (cond.op) {
    ...
    case 'incluye': return Array.isArray(v) ? v.map(String).includes(String(esperado)) : String(v ?? '') === String(esperado);
    case '=':
    default:        return String(v ?? '') === String(esperado ?? '');
  }
}

/* Qué campos se ven, de verdad, con estas respuestas.

   Se resuelve en cascada y no de una pasada: si A depende de B y B depende de
   C, ocultar C tiene que ocultar también A. Se repite hasta que nada cambia,
   con un tope de vueltas que además corta los ciclos (A↔B) — que se cortan al
   evaluar y no al guardar, porque una lista a medio reordenar puede pasar por
   un estado circular legítimo. */
function camposVisibles(campos, respuestas = {}) {
  ...
  for (let vuelta = 0; vuelta < lista.length + 1; vuelta++) {
    ...
    for (const c of lista) {
      ...
      /* Si aquella de la que depende está oculta, ésta tampoco se ve: su
         respuesta no se pudo dar. */
...
function validarFormulario(campos, respuestas = {}, ticketTypeId = null) {
  /* Sólo se exige lo que la persona pudo ver. Un campo oculto por su condición
     que siguiera siendo obligatorio dejaría el formulario imposible de enviar,
     y sin decir por qué: es el fallo clásico de esta función. */
...
function normalizarRespuestas(campos, respuestas = {}) {
  const out = {};
  /* Sólo se guarda lo de los campos que se veían. Si alguien contesta «rural»,
     rellena las preguntas del rural, y luego cambia a «urbana», las del rural
     siguen viajando en el payload — guardarlas dejaría respuestas a preguntas
     que esa persona no tenía delante, y el reporte contaría gente rural que
     dijo ser urbana. */
  for (const c of camposVisibles(campos, respuestas)) {
...
/* ── Cuántas preguntas caben. UNA sola respuesta para los tres formularios ──
 *
 ...
 * guardar.
 *
 * Y volvió a pasar dos veces más, porque el número se había copiado a mano en
 * otros dos sitios con topes propios: el torneo en 20 y el sub-evento en 12.
 * Los dos con su justificación escrita —«un torneo pide menos que el registro
 * del evento», «si alguien necesita treinta, lo que quiere es el formulario del
 * evento»— y las dos justificaciones eran una suposición sobre cómo trabaja la
 * gente. Un formulario de 21 preguntas para una batalla de pitch las desmintió:
 * describir una startup pide MÁS que comprar una entrada, no menos.
 *
 * Así que hay un solo número, aquí, y los tres formularios lo usan. Cuántas
 * preguntas necesita cada uno lo sabe quien lo escribe, no nosotros.
 *
 * ── Por qué no es infinito ──────────────────────────────────────────────
 *
 * No está para decirle a nadie cuántas preguntas necesita. Está porque la ruta
 * acepta la lista entera en un POST: sin tope, un bucle equivocado —o alguien
 * con un token— llena la tabla de filas por evento. Es un cortafuegos, no una
 * opinión, y por eso es generoso.
 */
const MAX_CAMPOS_FORMULARIO = 60;

/* Las columnas de una pregunta. `grupo` y `ayuda` los anade la 0055;
   `session_id`, la 0059; `sensible`, la 0115.

   ── Por que `sensible` estuvo aparte, y por que ya no ──────────────────

   Mientras la 0115 no estaba aplicada, `sensible` vivia en una segunda lista,
   `COLUMNAS_CAMPO_CON_SENSIBLE`, y los dos sitios que la necesitaban sabian
   caerse a esta si la columna no existia. No era manía: esta lista se usa en
   34 consultas, y meterle una columna que la base todavia no tiene las rompe
   TODAS en el hueco entre desplegar y migrar — y no con un error claro, sino
   con un formulario que deja de cargar en media plataforma.

   La 0115 esta aplicada y verificada, asi que la segunda lista ya no protege
   de nada: solo deja dos nombres para lo mismo, y que un formulario devuelva
   `sensible` o no segun cual de los dos use quien escriba la consulta. Eso es
   como se separan las cosas aqui. Se funden.

   Si vuelve a hacer falta anadir una columna nueva, el patron de las dos
   listas sigue siendo el correcto — mientras la migracion este pendiente. */
const COLUMNAS_CAMPO = 'id, tipo, etiqueta, opciones, requerido, orden, ticket_type_id, grupo, ayuda, buscable, visible_si, max_caracteres, max_palabras, sensible';

/* ── Las preguntas de un EVENTO, no de una actividad ─────────────────────
 *
 ...
 *
 * Distinguirlas es un `.is('session_id', null)` y estaba escrito a mano en cada
 * consulta: de las siete que había en las rutas públicas, sólo UNA lo llevaba.
 * Las otras servían al público las preguntas de los talleres mezcladas con las
 * de la compra, y el síntoma es el de siempre en este proyecto — falta algo y
 * no salta ningún error, sólo un formulario con más pasos de los que debería.
 *
 * Por eso se pide por aquí. Las dos consultas que NO deben filtrar son
 * legítimas y están documentadas donde viven: el adjunto que se busca por su
 * id —sirve igual para un campo de sesión— y la que va a buscar precisamente
 * las preguntas DE las sesiones.
 */
async function camposDelEvento(eventoId, { columnas = COLUMNAS_CAMPO, ordenar = true } = {}) {
  if (!eventoId) return [];
  const supabase = require('./supabase.js');
  let q = supabase.from('event_form_fields').select(columnas)
    .eq('evento_id', eventoId)
    .is('session_id', null);
  if (ordenar) q = q.order('orden', { ascending: true });
  const { data, error } = await q;
  /* El error se anota y se devuelve vacío: quien llama pinta un formulario, y
     hacer caer la página pública por esto sería peor. Pero en silencio no: una
     consulta fallida aquí es un formulario que no pregunta nada. */
  if (error) console.error(`[formulario] campos de ${eventoId}: ${error.message}`);
  return data || [];
}
...
function filaCampo(c, orden) {
  return {
    ...
    /* Sólo se guarda una condición completa. Una a medias —sin campo, o con un
       operador que no existe— se descarta en vez de guardarse rota: una
       condición que no se puede evaluar dejaría la pregunta oculta para
       siempre sin que nadie entienda por qué. */
    /* Solo donde aplica y solo un entero positivo. Un limite que llega como
       texto vacio, cero o basura vuelve a null —sin limite— en vez de guardarse
       roto: un `0` guardado seria una pregunta imposible de responder. Y si el
       tipo deja de ser texto, el limite se va con el en vez de quedarse
       escondido esperando a que alguien lo devuelva a texto. */
    max_caracteres: TIPOS_CON_LIMITE.has(c.tipo) ? enteroPositivo(c.max_caracteres, 10000) : null,
    max_palabras:   TIPOS_CON_LIMITE.has(c.tipo) ? enteroPositivo(c.max_palabras, 2000)   : null,
    /* Si lo que se pide son datos sensibles. Sólo significa algo en un archivo
       —decide a qué bucket va y si el enlace caduca— y por eso se limpia en
       cualquier otro tipo: un `sensible: true` colgado de un campo de texto es
       una promesa que nadie cumple, y quien lo lea creerá que ese dato está
       protegido. */
    sensible: c.tipo === 'archivo' ? Boolean(c.sensible) : false,
    visible_si: condicionValida(c.visible_si) ? {
      campo: String(c.visible_si.campo),
      op: OPERADORES_CONDICION.includes(c.visible_si.op) ? c.visible_si.op : '=',
      valor: c.visible_si.valor ?? '',
    } : null,
  };
}

/* El mismo tope que el CHECK de la 0107: si aqui pasara algo mayor, el
   guardado fallaria con el error crudo de Postgres en vez de con una frase. */
function enteroPositivo(v, tope) {
  const n = Math.trunc(Number(v));
  if (!Number.isFinite(n) || n <= 0) return null;
  return Math.min(n, tope);
}

const OPERADORES_CONDICION = ['=', '!=', 'incluye'];

function condicionValida(v) {
  return Boolean(v && typeof v === 'object' && v.campo);
}
...
function validarDefinicion(campos, { max = MAX_CAMPOS_FORMULARIO } = {}) {
  ...
  /* El tope se cuenta POR BOLETA, no por evento.

     Se escribió cuando el formulario era uno solo para todos, y entonces las
     dos cosas eran lo mismo. Desde que cada tipo de boleta tiene el suyo, el
     tope global castiga justo lo que queremos: un evento con cuatro boletas de
     quince preguntas cada una es perfectamente razonable —nadie llena más de
     quince— y con el conteo viejo lo rechazaba por «sesenta».

     Lo que importa es cuántas ve UNA persona, y una persona ve las de su
     boleta más las que se piden a todas. */
  const compartidas = campos.filter(c => !c.ticket_type_id).length;
  const porBoleta = new Map();
  for (const c of campos) {
    if (!c.ticket_type_id) continue;
    porBoleta.set(c.ticket_type_id, (porBoleta.get(c.ticket_type_id) || 0) + 1);
  }
  const mayor = porBoleta.size
    ? Math.max(...[...porBoleta.values()].map(n => n + compartidas))
    : compartidas;
  if (mayor > max) return `Máximo ${max} preguntas por boleta. Una de las tuyas llegaría a ${mayor}.`;
  for (const c of campos) {
