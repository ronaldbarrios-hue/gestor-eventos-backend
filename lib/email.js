...
let _fallbackMode = null;     // 'gmail_oauth' | 'resend' | null — sólo si NINGÚN buzón SMTP está puesto
...
function init() {
  ...
  for (const [id, sufijo] of [['cpanel', ''], ['smtp2', '2']]) {
    ...
    try {
      ...
      _buzones.push({ id, host, port, user, transport });
...
function direccionReal() {
  if (_buzones.length) {
    /* Con dos buzones alternando, el remitente técnico cambia según a cuál le
       tocó el turno — `EMAIL_FROM` lo fija para que no varíe de un correo a
       otro; sin ella se usa el del primer buzón como referencia. */
