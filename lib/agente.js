...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Crea un bloque/sesión en la agenda de un evento. Puede vincularse a un speaker por su nombre.',
    input_schema: {
      type: 'object',
      properties: {
        evento_id: { type: 'string', description: 'UUID del evento.' },
  ...
  {
    name: 'listar_agenda',
    description: 'Lista las sesiones de la agenda de un evento, ordenadas, con su speaker vinculado.',
    input_schema: {
      type: 'object',
      properties: { evento_id: { type: 'string', description: 'UUID del evento.' } },
      required: ['evento_id'],
  ...
  {
    name: 'ver_mi_perfil',
    description: 'Muestra el perfil y plan del organizador autenticado (nombre, email, empresa, plan y vigencia).',
    input_schema: { type: 'object', properties: {} },
  },
  {
    name: 'solicitar_formulario',
    description: 'Cuando necesites VARIOS datos del usuario para ejecutar una acción (ej. crear un evento, agregar un speaker), NO preguntes en texto: usa esta herramienta para enviarle un formulario con los campos EN ORDEN. El usuario lo llena y te responde con los valores; luego ejecutas la acción real con la herramienta que corresponda.',
    input_schema: {
      type: 'object',
      properties: {
...
const EXECUTORS = {
  ...
  async ver_mi_perfil(userId) {
    const { data, error } = await supabase
      .from('profiles')
      .select('nombre, email, empresa, plan, plan_expires_at')
    ...
    return {
      ...
      plan: data.plan || 'free',
      plan_vigente: esPro ? 'pro activo' : (data.plan === 'pro' ? 'pro vencido' : 'free'),
      expira: data.plan_expires_at || null,
    };
  },
};
...
/* ── Router de herramientas ─────────────────────────────────────────
   Enviar las 60 tools en cada request revienta los límites diarios de
...
const TRIGGERS = {
  ...
  equipo: ['listar_equipo', 'invitar_miembro', 'quitar_miembro', 'crear_rol'],
...
async function chat(userId, history, archivos) {
  if (PROVIDERS.length === 0) {
    return {
      reply: 'El asistente IA no está configurado. Define GROQ_API_KEY (gratis), GEMINI_API_KEY (gratis) o ANTHROPIC_API_KEY.',
      mood: 'error', acciones: [],
    };
  }
