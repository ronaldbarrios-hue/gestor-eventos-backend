/* GESTEK — Recordatorios automáticos de eventos.
   Dos sistemas en un mismo cron periódico:
...
const { sendMail, plantillaRecordatorioEvento } = require('./email.js');
...
async function correrCicloEmail() {
  ...
  for (const r of pendientes) {
    try {
      const html = plantillaRecordatorioEvento({
        nombre: r.guest_nombre,
        tipo: r.tipo,
        eventoTitulo: r.evento_titulo,
        eventoFecha: r.evento_inicio,
        eventoLugar: r.evento_location,
        codigo: r.codigo,
        linkEvento: `${FRONTEND_URL}/explorar/${r.evento_slug}`,
      });

      const resultado = await sendMail({
        to: r.guest_email,
        subject: `${r.evento_titulo} — recordatorio`,
        html,
      });
      ...
      await supabase.from('email_log').insert({
        ...
        error: resultado.ok ? null : (resultado.error || 'envío falló'),
      });
...
function iniciarCronRecordatorios() {
  ...
  console.log('[recordatorios] cron programado cada 15 minutos (push + email).');
}
