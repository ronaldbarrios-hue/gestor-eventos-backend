...
CREATE INDEX `idx_agenda_sessions_subcategoria` ON `agenda_sessions` (`evento_id`, `subcategoria`);  -- era parcial: WHERE (subcategoria IS NOT NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `audit_log_actor_idx` ON `audit_log` (`actor_id`);  -- era parcial: WHERE (actor_id IS NOT NULL) (no único: la condición se puede tirar)
...
-- A MANO (único parcial): catalogo_roles_slug_global_uidx — CREATE UNIQUE INDEX catalogo_roles_slug_global_uidx ON public.catalogo_roles USING btree (slug) WHERE (global = true)
CREATE UNIQUE INDEX `categorias_slug_key` ON `categorias` (`slug`);
CREATE INDEX `chat_channel_prefs_user_idx` ON `chat_channel_prefs` (`user_id`);  -- era parcial: WHERE (archivado = false) (no único: la condición se puede tirar)
-- A MANO (único parcial): chat_channels_dm_uidx — CREATE UNIQUE INDEX chat_channels_dm_uidx ON public.chat_channels USING btree (evento_id, dm_key) WHERE (dm_key IS NOT NULL)
...
CREATE INDEX `chat_messages_canal_vivos_idx` ON `chat_messages` (`channel_id`, `created_at`);  -- era parcial: WHERE (borrado_at IS NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `idx_form_fields_torneo` ON `event_form_fields` (`torneo_id`, `orden`);
...
CREATE INDEX `event_members_evento_user_idx` ON `event_members` (`evento_id`, `user_id`);  -- era parcial: WHERE (status = 'active'::text) (no único: la condición se puede tirar)
CREATE INDEX `event_members_user_idx` ON `event_members` (`user_id`);  -- era parcial: WHERE (user_id IS NOT NULL) (no único: la condición se puede tirar)
CREATE INDEX `event_requests_autor_idx` ON `event_requests` (`autor_id`);
CREATE INDEX `event_requests_cambio_abierto_idx` ON `event_requests` (`evento_id`, `created_at`);  -- era parcial: WHERE ((tipo = 'cambio'::text) AND (estado = 'abierta'::text)) (no único: la condición se puede tirar)
CREATE INDEX `event_requests_estado_idx` ON `event_requests` (`evento_id`);  -- era parcial: WHERE (estado <> 'resuelta'::text) (no único: la condición se puede tirar)
...
CREATE INDEX `waitlist_oferta_expira_idx` ON `event_waitlist` (`oferta_expira`);  -- era parcial: WHERE (oferta_expira IS NOT NULL) (no único: la condición se puede tirar)
-- A MANO (único parcial): waitlist_oferta_token_uidx — CREATE UNIQUE INDEX waitlist_oferta_token_uidx ON public.event_waitlist USING btree (oferta_token) WHERE (oferta_token IS NOT NULL)
...
CREATE INDEX `evento_email_envios_fallidos_idx` ON `evento_email_envios` (`evento_id`);  -- era parcial: WHERE (ok = false) (no único: la condición se puede tirar)
...
CREATE INDEX `eventos_estado_idx` ON `eventos` (`estado`);  -- era parcial: WHERE (deleted_at IS NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `idx_oauth_tokens_expira` ON `oauth_tokens` (`expira_at`);  -- era parcial: WHERE (NOT revocado) (no único: la condición se puede tirar)
...
CREATE INDEX `payment_tx_user_idx` ON `payment_transactions` (`user_id`);  -- era parcial: WHERE (user_id IS NOT NULL) (no único: la condición se puede tirar)
CREATE INDEX `perfil_talento_publicado_idx` ON `perfil_talento` (`publicado`);  -- era parcial: WHERE (publicado = true) (no único: la condición se puede tirar)
CREATE UNIQUE INDEX `perfil_talento_user_id_key` ON `perfil_talento` (`user_id`);
CREATE INDEX `idx_points_log_origen` ON `points_log` (`origen_tipo`, `origen_id`);  -- era parcial: WHERE (origen_tipo IS NOT NULL) (no único: la condición se puede tirar)
CREATE INDEX `idx_points_log_usuario_evento` ON `points_log` (`user_id`, `evento_id`, `created_at`);
CREATE INDEX `points_log_evento_aud_idx` ON `points_log` (`evento_id`, `audiencia`);  -- era parcial: WHERE (evento_id IS NOT NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `sugerencias_catalogo_user_idx` ON `sugerencias_catalogo` (`user_id`);  -- era parcial: WHERE (user_id IS NOT NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `tareas_asignado_rol_idx` ON `tareas` (`asignado_rol_id`);  -- era parcial: WHERE (asignado_rol_id IS NOT NULL) (no único: la condición se puede tirar)
CREATE INDEX `tareas_asignado_user_idx` ON `tareas` (`asignado_user_id`);  -- era parcial: WHERE (asignado_user_id IS NOT NULL) (no único: la condición se puede tirar)
...
CREATE INDEX `idx_zonas_evento_tipo` ON `zonas` (`evento_id`, `tipo`);
CREATE INDEX `zonas_evento_idx` ON `zonas` (`evento_id`, `orden`);
