 ...
 *
 * · La zona se identifica por su `id` de `page_json.zonas`, no por el nombre.
...
async function zonasDelEvento(eventoId) {
  ...
  const zonas = Array.isArray(data?.page_json?.zonas) ? data.page_json.zonas : [];
  return zonas
    .filter(z => z && z.id)
    .map(z => ({ id: String(z.id), nombre: String(z.nombre || '').trim(), aforo_max: Number(z.aforo_max) || null }));
}
...
/* Último corte por clave de zona. */
...
async function ocupacion(eventoId, zonasDadas = null) {
  const zonas = zonasDadas || await zonasDelEvento(eventoId);
  ...
  return zonas.map(z => {
    ...
    return {
      ...
      ocupacion_pct: z.aforo_max ? Math.round((dentro / z.aforo_max) * 100) : null,
...
module.exports = { zonasDelEvento, ocupacion, cortesDelEvento, agregadoManual, juntar };
