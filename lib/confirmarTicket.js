...
const { emitirPuestos, modoDelTipo } = require('./emitirPuestos.js');
const { personasDeTicket } = require('./cuantasPersonas.js');
...
async function confirmarTicketPagado(ticketId, monto, via) {
  ...
  if (evWh?.owner_id) {
    ...
    /* El aforo cuenta PERSONAS, no boletas. Una mesa de ringside es una boleta
       y cuatro personas; sumar de uno en uno dejaba al organizador creyendo que
       le quedaban sitios que no existen. Ver `lib/cuantasPersonas.js`. */
    const personas = await personasDeTicket(ticketId);
    await supabase.from('eventos')
      .update({ aforo_vendido: (evWh.aforo_vendido || 0) + personas })
      .eq('id', ticket.evento_id);
  }

  /* Los puestos de la boleta (0118). Va AQUÍ y no en cada pasarela porque éste
     es el único paso por el que pasan todas las boletas de pago —Wompi,
     MercadoPago y la cortesía del agente—, y además está cerrado contra
     reintentos por el `update ... neq('estado','pagado')` de arriba: un webhook
     repetido no crea los puestos dos veces.

     Una boleta de una persona no crea ninguno: es lo que la plataforma ya
     hacía. */
  const cuantas = await personasDeTicket(ticketId);
  if (cuantas > 1) {
    await emitirPuestos({
      ticket: { id: ticketId, codigo: tFull?.codigo },
      eventoId: ticket.evento_id, personas: cuantas, modo: await modoDelTipo(tFull?.ticket_type_id),
      titular: { nombre: tFull?.guest_nombre, email: tFull?.guest_email },
    });
  }
