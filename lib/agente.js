...
const { avisosDePublicacion } = require('./avisosDePublicacion.js');
const { normalizarModalidad } = require('./modalidad.js');
const { anotarAccionDelAgente } = require('./auditarAgente.js');
const espaciosLib = require('./espacios.js');
const { fallaPaginas, catalogoPublico, MAX_BLOQUES_POR_PAGINA, MAX_PAGINAS } = require('./bloquesLanding.js');
...
/* El envio de correo NO se hace aqui: pasa por lib/emailPlantillas.js, que es
   el unico sitio que sabe de marca del evento, registro, cola, buzon propio y
   calendario. Antes este archivo importaba sendMail y se saltaba todo eso. */
const { enviarEmailEvento } = require('./emailPlantillas.js');

/* Tope de destinatarios por recordatorio. Existia como un `.slice(0, 200)`
   suelto, sin nombre y sin avisar a nadie de que habia recortado. */
const TOPE_RECORDATORIO = 200;
...
const { boletaQueYaTenia } = require('./yaEstabaRegistrado.js');
const { esUrlImagenSegura } = require('./urls.js');
const { COLS_TARJETA, conZona } = require('./expositores.js');
const { zonasDelEvento } = require('./aforoZonas.js');
const { generarCodigo } = require('./codigos.js');
...
const TOOLS = [
  ...
  /* ── El plano de venta ──────────────────────────────────────────────────
   *
   * Se podía montar un concierto entero hablando con el agente —evento,
   * boletas, artistas, agenda, descuentos— y no lo que hace que sea un
   * concierto: dónde se sienta la gente. El módulo existía y el agente no
   * llegaba a él.
   *
   * Tres herramientas y no ocho: crear una zona, llenarla de sitios, y mirar
   * cómo va. Editar y borrar sitio a sitio se hace en el panel, donde se ve lo
   * que se está tocando — pedirle a un modelo que borre «la fila G» sobre 124
   * unidades es invitarle a acertar. */
  {
    name: 'crear_zona_plano',
    description: 'Crea una zona del plano de venta de un evento: una gradería por aforo, o una sección donde después se generan sitios (platea, pista, palcos). Para conciertos, teatro, boxeo o cualquier evento donde se compra un sitio concreto.',
    input_schema: {
      type: 'object',
      properties: {
        evento_id: { type: 'string', description: 'UUID del evento.' },
        nombre: { type: 'string', description: 'Nombre de la zona: «Platea», «Pista VIP», «Gradería Norte».' },
        modo: {
          type: 'string', enum: ['aforo', 'vendible'],
          description: 'aforo = cuenta gente sin asignar sitio (una gradería). vendible = dentro se generan sitios que se compran uno a uno. Por defecto aforo.',
        },
        aforo_max: { type: 'number', description: 'Cuántas personas caben. Sólo con modo aforo.' },
        tipo: { type: 'string', description: 'Cómo lo llama el recinto: seccion, graderia, palco… (opcional).' },
      },
      required: ['evento_id', 'nombre'],
    },
  },
  {
    name: 'generar_sitios',
    description: 'Genera de una vez los sitios que se venden dentro de una zona: «8 filas de 14 sillas» o «12 mesas de 4». Los nombra como están en el suelo (Fila A1, Fila A2… o Mesa 1 si es una sola fila) y les pone coordenadas para el mapa. Si se le pasa el tipo de boleta, además les asigna el precio.',
    input_schema: {
      type: 'object',
      properties: {
        evento_id: { type: 'string', description: 'UUID del evento.' },
        zona: { type: 'string', description: 'Nombre de la zona donde van (tiene que existir ya).' },
        filas: { type: 'number', description: 'Cuántas filas. 1 para mesas o palcos sueltos.' },
        por_fila: { type: 'number', description: 'Cuántos sitios por fila.' },
        tipo: { type: 'string', description: 'Cómo se llama cada uno: silla, mesa, palco. Por defecto silla.' },
        capacidad: { type: 'number', description: 'Cuántas personas admite cada sitio. Una silla 1, una mesa 4. Por defecto 1.' },
        tipo_boleta: { type: 'string', description: 'Nombre del tipo de boleta con el que se compran. Sin esto, los sitios se ven en el mapa y NADIE los puede comprar.' },
      },
      required: ['evento_id', 'zona', 'por_fila'],
    },
  },
  {
    name: 'ver_plano',
    description: 'Cómo va el plano de venta de un evento: las zonas, cuántos sitios tiene cada una, cuántos están vendidos o retenidos, y cuáles no tienen precio asignado.',
    input_schema: {
      type: 'object',
  ...
  {
    name: 'catalogo_bloques_landing',
    description: 'Los tipos de bloque con los que se arma la landing de un evento, y qué campos admite cada uno. Consúltalo ANTES de armar o editar una página: es el contrato, y el servidor rechaza lo que no encaje.',
    input_schema: { type: 'object', properties: {} },
  },
  {
    name: 'ver_landing',
    description: 'Devuelve la landing de un evento tal cual está: sus páginas y los bloques de cada una, con sus datos. Es lo que hay que leer antes de modificarla para no pisar lo que ya estaba.',
    input_schema: {
      type: 'object',
      properties: { evento_id: { type: 'string', description: 'UUID del evento.' } },
      required: ['evento_id'],
  ...
  },
  {
    ...
    description: 'Reemplaza la landing de un evento por las páginas que se le pasen. CUIDADO: sustituye, no añade — hay que mandar la página COMPLETA, así que primero se lee con ver_landing. Cada bloque es { type, data } y tiene que encajar con catalogo_bloques_landing; si no, se rechaza entera y no se guarda nada.',
    input_schema: {
      type: 'object',
      properties: {
        evento_id: { type: 'string', description: 'UUID del evento.' },
        paginas: {
          type: 'array',
          description: 'Las páginas de la landing. Cada una: { nombre?, blocks: [ { type, data } ] }.',
          items: { type: 'object' },
        },
      },
      required: ['evento_id', 'paginas'],
    },
  },
  {
...
const EXECUTORS = {
  ...
  async crear_evento(userId, input) {
    ...
    const insert = {
      ...
      /* Se traduce a lo que guarda la plataforma. «Presencial» es como habla
         la gente; `fisico` es lo que comparan el editor y los avisos. Escribir
         «presencial» dejaba el evento sin campos de lugar en su propio editor
         y con un aviso diciendo que era en línea. */
      modalidad: normalizarModalidad(input.modalidad),
    };
  ...
  async publicar_evento(userId, input) {
    try {
      ...
      /* Ya publicado: se devuelven los avisos igual.
       *
       * Antes se salía con un `ok` limpio, y eso es justo al revés de lo que
       * hace falta: los avisos describen el ESTADO de hoy —«hay boletas de pago
       * y ninguna cuenta de cobro»—, no el acto de publicar. Un evento ya
       * publicado con ese problema lo tiene AHORA, y encima lleva tiempo
       * teniéndolo.
       *
       * Salió al preguntar por el concierto de prueba: contestó `ok: true,
       * ya_publicado: true` y nada más, con 124 boletas de pago y cero
       * pasarelas conectadas. */
      if (ev.estado === 'publicado') {
        const { data: completo } = await supabase
          .from('eventos')
          .select('id, titulo, slug, estado, owner_id, fecha_inicio, location_nombre, modalidad, url_virtual')
          .eq('id', ev.id).maybeSingle();
        return {
          ok: true, ya_publicado: true, evento_id: ev.id,
          avisos: await avisosDePublicacion(ev.id, completo || ev),
        };
      }
      const { data, error } = await supabase
        .from('eventos')
        ...
        /* Hacen falta para los avisos: sin `owner_id` no se puede mirar si hay
           cuenta de cobro, y sin lugar ni modalidad no se puede avisar de lo
           que falta. */
        .select('id, titulo, slug, estado, owner_id, fecha_inicio, location_nombre, modalidad, url_virtual')
        .single();
      if (error) return { error: error.message };
       ...
       *
       * Antes esta herramienta no devolvía ninguno: publicaba y ya. Comprobado
       * publicando un concierto de prueba desde el agente —124 boletas de pago,
       * cero cuentas de cobro— y no dijo nada. O sea que el camino más rápido
       * para publicar era el único que no avisaba.
       *
       * El cálculo es el MISMO que el del panel, no una copia: si se separaran,
       * publicar por un camino avisaría de cosas distintas que por el otro. */
      const avisos = await avisosDePublicacion(data.id, data);

      return {
        ok: true,
        evento: { id: data.id, titulo: data.titulo, slug: data.slug, estado: data.estado },
        url_publica: `/explorar/${data.slug}`,
        avisos,
      };
  ...
  /* ── El plano de venta ────────────────────────────────────────────────
   *
   * Todo lo que valida y nombra vive en `lib/espacios.js`, el mismo que usan
   * las rutas del panel. Reescribirlo aquí haría que montar un plano hablando
   * aceptara cosas que la pantalla rechaza —o al revés—, y esa diferencia no
   * se ve hasta que alguien compara. */

  async crear_zona_plano(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const fila = espaciosLib.filaEspacio({
        nombre: input.nombre,
        tipo: input.tipo || (input.modo === 'vendible' ? 'seccion' : 'zona'),
        /* Una zona que va a contener sitios se crea en modo aforo: lo vendible
           son sus hijos, no ella. Pedir «vendible» aquí casi siempre quiere
           decir «aquí dentro se vende», no «se vende esta zona entera». */
        modo: 'aforo',
        aforo_max: input.aforo_max,
      }, ev.id);

      const malo = espaciosLib.validarEspacio(fila);
      if (malo) return { error: malo };

      const { data, error } = await supabase
        .from('espacios').insert(fila).select(espaciosLib.COLUMNAS).single();
      if (error) return { error: error.message };
      return {
        ok: true,
        zona: { id: data.id, nombre: data.nombre, modo: data.modo, aforo_max: data.aforo_max },
        siguiente: input.modo === 'vendible' || !input.aforo_max
          ? `Ahora genera los sitios con generar_sitios, zona: "${data.nombre}".`
          : null,
      };
    } catch (e) { return { error: e.message }; }
  },

  async generar_sitios(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);

      const { data: zonas, error: eZ } = await supabase
        .from('espacios').select('id, nombre, modo')
        .eq('evento_id', ev.id).is('parent_id', null);
      if (eZ) return { error: eZ.message };

      /* Por nombre y sin distinguir tildes ni mayúsculas: quien habla escribe
         «pista vip», no el UUID. */
      const limpia = (x) => String(x || '').normalize('NFD').replace(/[̀-ͯ]/g, '').trim().toLowerCase();
      const zona = (zonas || []).find(z => limpia(z.nombre) === limpia(input.zona));
      if (!zona) {
        return { error: `No hay ninguna zona llamada «${input.zona}». Las que hay: ${(zonas || []).map(z => z.nombre).join(', ') || 'ninguna'}.` };
      }

      const { unidades, error: malo } = espaciosLib.generarUnidades({
        filas: input.filas || 1,
        porFila: input.por_fila,
        tipo: input.tipo || 'silla',
        capacidad: input.capacidad || 1,
      });
      if (malo) return { error: malo };

      const filas = unidades.map(u => ({
        ...espaciosLib.filaEspacio(u, ev.id), parent_id: zona.id,
      }));
      const { data, error } = await supabase.from('espacios').insert(filas).select('id');
      if (error) return { error: error.message };

      /* El precio, si se dijo con qué boleta se compran.
       *
       * Sin esto los sitios se ven en el mapa y NADIE los puede comprar: el
       * servidor no sabe qué cobrar, y la venta no falla — simplemente no
       * ocurre. Se avisa aquí y no se deja para que lo descubra el aviso de
       * publicar, que llega mucho después. */
      let localidad = null;
      if (input.tipo_boleta) {
        const { data: tipos } = await supabase
          .from('ticket_types').select('id, nombre').eq('evento_id', ev.id);
        const t = (tipos || []).find(x => limpia(x.nombre) === limpia(input.tipo_boleta))
              || (tipos || []).find(x => limpia(x.nombre).includes(limpia(input.tipo_boleta)));
        if (!t) {
          return {
            ok: true, creados: data?.length || 0,
            aviso: `Se crearon los sitios, pero no hay ninguna boleta llamada «${input.tipo_boleta}»: por ahora nadie los puede comprar. Las que hay: ${(tipos || []).map(x => x.nombre).join(', ') || 'ninguna'}.`,
          };
        }
        await supabase.from('ticket_type_espacios')
          .insert(data.map(e => ({ ticket_type_id: t.id, espacio_id: e.id })));
        localidad = t.nombre;
      }
      ...
      return {
        ...
        zona: zona.nombre,
        se_compran_con: localidad,
        aviso: localidad ? null : 'Estos sitios no tienen tipo de boleta: se ven en el mapa y nadie los puede comprar. Vuelve a llamar con tipo_boleta.',
      };
    } catch (e) { return { error: e.message }; }
  },

  async ver_plano(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      ...
      if (error) return { error: error.message };
      if (!espacios?.length) return { ok: true, hay_plano: false };

      const { data: reservas, error: eR } = await supabase
        .from('espacio_reservas').select('espacio_id, estado, expira_at')
        .eq('evento_id', ev.id).in('estado', ['retenido', 'vendido']);
      if (eR) return { error: eR.message };

      const { data: loc } = await supabase
        .from('ticket_type_espacios').select('espacio_id')
        .in('espacio_id', espacios.filter(e => e.modo === 'vendible').map(e => e.id));
      const conPrecio = new Set((loc || []).map(l => l.espacio_id));

      const mapa = espaciosLib.mapaPublico({ espacios, reservas });
      const libres = new Map(mapa.map(u => [u.id, u.libre]));

      const zonas = espacios.filter(e => !e.parent_id).map(z => {
        const dentro = espacios.filter(e => e.parent_id === z.id && e.modo === 'vendible');
        return {
          nombre: z.nombre,
          aforo_max: z.aforo_max,
          sitios: dentro.length,
          libres: dentro.filter(e => libres.get(e.id)).length,
          sin_precio: dentro.filter(e => !conPrecio.has(e.id)).length,
        };
      });

      const sinPrecio = zonas.reduce((n, z) => n + z.sin_precio, 0);
      return {
        ok: true, hay_plano: true, zonas,
        aviso: sinPrecio
          ? `${sinPrecio} ${sinPrecio === 1 ? 'sitio no tiene' : 'sitios no tienen'} tipo de boleta: se ven en el mapa y nadie los puede comprar.`
          : null,
      };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async enviar_recordatorio(userId, input) {
    try {
      ...
      const destinatarios = [...new Set((tks || []).map(t => t.guest_email).filter(Boolean))];
      const emails = destinatarios.slice(0, TOPE_RECORDATORIO);
      ...
      /* Esto armaba su propio HTML —morado y azul marino fijos— y lo mandaba
         con sendMail a pelo. Tres consecuencias, todas invisibles:

         · Salía sin la marca del evento, que es justo el problema que existe
           lib/emailPlantillas.js para resolver (su cabecera lo cuenta).
         · No quedaba registrado en evento_email_envios, ni pasaba por la cola,
           ni por el buzón propio del organizador, ni llevaba el .ics — que es
           la pieza central de la estrategia de recordatorios.
         · `enviados++` contaba las llamadas que no lanzaban. sendMail NUNCA
           lanza: devuelve {ok:false} cuando falla. Así que el contador daba
           siempre el total, y decía «enviados: 1» con el correo rechazado.

         Ahora va por el mismo camino que las campañas del panel. */
      const plantilla = {
        asunto:     `Recordatorio: ${ev.titulo}`,
        encabezado: ev.titulo,
        cuerpo:     String(input.mensaje).trim(),
      };
      ...
      for (let i = 0; i < emails.length; i += LOTE) {
        await Promise.all(emails.slice(i, i + LOTE).map(async (to) => {
          const r = await enviarEmailEvento({
            evento: ev.id, tipo: 'personalizado', to, plantilla, prioridad: 5,
          });
          if (r.ok) enviados++;
          else fallos.push({ email: to, motivo: r.motivo });
        }));
      }

      /* El recorte a 200 estaba escondido en un `.slice()` y no se contaba en
         ningún sitio: en un evento de 7.000 personas esto habría avisado a 200
         y devuelto «ok» tan tranquilo. */
      const sinAvisar = destinatarios.length - emails.length;
      ...
      return {
        ...
        emails_fallidos: fallos.length,
        total_destinatarios: Math.max(userIds.length, emails.length),
        ...(fallos.length ? { fallos: fallos.slice(0, 10) } : {}),
        ...(sinAvisar > 0 ? {
          advertencia: `Se avisó a ${emails.length} personas; ${sinAvisar} quedaron fuera por el tope de ${TOPE_RECORDATORIO} por envío. Repite para el resto o usa las campañas del panel.`,
        } : {}),
      };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async duplicar_evento(userId, input) {
    try {
      ...
      const { data: tt } = await supabase
        .from('ticket_types')
        .select('nombre, precio, currency, cupo, early_bird_precio, early_bird_hasta, venta_hasta, orden, activo')
        .eq('evento_id', ev.id);
      let boletas = 0;
      if (tt && tt.length) {
  ...
  async exportar_asistentes_csv(userId, input) {
    try {
      ...
      /* El tope existe por una razon real: un CSV de 7.000 filas dentro de una
         respuesta de herramienta no cabe. Lo que estaba mal no era el limite,
         era que NO SE DECIA: con 7.000 asistentes devolvia 2.000 y el
         organizador se llevaba un archivo que parecia completo.

         Asi que se cuenta primero y se avisa. Para llevarse todo —y ademas con
         las respuestas del formulario, que esta via no incluye— esta la
         exportacion del panel. */
      const TOPE = 2000;
      const { count: total } = await supabase
        .from('tickets').select('id', { count: 'exact', head: true }).eq('evento_id', ev.id);
      ...
      const faltan = Math.max(0, (total || 0) - rows.length);
      return {
        evento: ev.titulo,
        filas: rows.length,
        total_en_el_evento: total ?? rows.length,
        csv,
        ...(faltan > 0 ? {
          advertencia: `Van ${rows.length} de ${total}. Faltan ${faltan}: no caben en una respuesta. Para llevarte todo, y ademas con las respuestas del formulario, usa Exportar Excel en la pestana de asistentes del panel.`,
        } : {}),
        nota: 'Este CSV no incluye las respuestas del formulario. La exportacion del panel si.',
      };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async emitir_cortesia(userId, input) {
    try {
      ...
      const email = input.email.toLowerCase().trim();
      /* La MISMA regla que el registro publico y que la importacion, y vive en
       * un solo sitio (`lib/yaEstabaRegistrado.js`).
       *
       * Aqui habia una tercera, y era la mas estricta de las tres: rechazaba
       * cualquier correo con una boleta en el evento, sin mirar el tipo, ni el
       * nombre, ni el estado. O sea que no se podia emitir una cortesia para
       * el DemoDay a alguien del registro general, ni volver a emitirsela a
       * quien se le habia invalidado la suya — que es cuando de verdad se pide
       * una cortesia.
       *
       * Tres caminos con tres reglas para la misma pregunta. Ahora una. */
      const yaTenia = await boletaQueYaTenia({
        eventoId: ev.id, tipoId: tipo.id, email, nombre: input.nombre,
      });
      if (yaTenia) {
        return { error: `${input.nombre.trim()} ya tiene una boleta de "${tipo.nombre}" (${yaTenia.codigo}).` };
      }

      const pagada = input.marcar_pagada !== false;
      const codigo = generarCodigo();

      /* Se inserta como 'emitido' aunque vaya a quedar pagada: quien hace la
         transición a pagada es confirmarTicketPagado, y esa función se planta
         si la boleta YA está pagada (es idempotente a propósito). Insertarla
         pagada de entrada la dejaba fuera de toda la cadena de efectos. */
      ...
      /* El QR se firma ANTES de confirmar: confirmarTicketPagado lee qr_token
         de la fila para meterlo en el correo. Al revés, la invitación salía
         sin código que escanear. */
      ...
      /* Aquí estaba el agujero: esto emitía la boleta y no avisaba a nadie.
         Ni correo con el QR, ni webhook, ni aforo. Y una cortesía es
         exactamente el caso donde la persona ESPERA recibir algo: la invitas,
         y no le llega nada.

         En vez de repetir esos cuatro pasos, se delega en la misma función
         que usan Wompi y Mercado Pago, que ya los hace todos —incluido subir
         `vendidos`, por eso aquí ya no se toca. */
      let correo = null;
      if (pagada) {
        const { confirmarTicketPagado } = require('./confirmarTicket.js');
        await confirmarTicketPagado(ticket.id, 0, 'cortesia');

        /* El envío va sin esperar respuesta, así que aquí NO se puede decir
           «enviado». Lo que sí se sabe es si hay proveedor: sin él los correos
           se descartan en silencio, y quien emite la cortesía se merece
           enterarse ahora y no el día del evento. */
        const { diagnosticoProveedor } = require('./emailPlantillas.js');
        correo = diagnosticoProveedor().configurado
          ? `Invitación en camino a ${email}, con el QR.`
          : `OJO: no hay correo configurado en el servidor, así que ${email} NO va a recibir nada. La boleta es válida — pásale el código ${ticket.codigo} a mano, o usa el reparto sin correo para imprimirla.`;
      } else {
        await supabase.from('ticket_types')
          .update({ vendidos: (tipo.vendidos || 0) + 1 }).eq('id', tipo.id);
        correo = 'La boleta queda sin confirmar, así que NO se envió invitación.';
      }

      return { ok: true, codigo: ticket.codigo, invitado: input.nombre.trim(), tipo: tipo.nombre, correo };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async crear_codigo_descuento(userId, input) {
    try {
      ...
      /* Va a `promociones`, que es la tabla que lee el cobro.
         Antes esto escribía en `discount_codes`, una tabla paralela que no
         aplica nadie: el agente contestaba «código creado» y quien compraba
         escribía el código y le decían que no existe. La plataforma se
         contradecía según por dónde entraras. Ver la migración 0100.

         El vocabulario de fuera se conserva —`percent`/`fixed` es lo que
         entiende la herramienta y lo que ya está en las conversaciones— y se
         traduce aquí. */
      const { data, error } = await supabase
        .from('promociones').insert({
          evento_id: ev.id,
          codigo: input.codigo.trim().toUpperCase(),
          tipo: input.tipo === 'fixed' ? 'fijo' : 'porcentaje',
          valor,
          limite_usos: (input.max_usos == null || input.max_usos === '') ? null : Number(input.max_usos),
          vigente_hasta: input.expira_at || null,
          activo: true,
          descripcion: 'Creado desde el asistente',
        }).select('id, codigo, tipo, valor, limite_usos, vigente_hasta').single();
      if (error) {
  ...
  /* ── La landing como datos ──────────────────────────────────────────────

     Lo pedido era que se pudiera «armar la página pidiéndoselo a Claude». Eso
     sólo se sostiene si hay un contrato: un modelo genera JSON contra un
     esquema de forma fiable, y si se equivoca el servidor se lo dice. Con HTML
     libre no habría forma de saber si lo generado está bien hasta que alguien
     abre la página — y además sería XSS con el origen del evento.

     El contrato vive en lib/bloquesLanding.js y es el MISMO que valida el
     editor visual al guardar. Uno solo, a propósito: mantener dos catálogos es
     el error que ya costó caro dos veces en este proyecto. */

  async catalogo_bloques_landing() {
    return {
      bloques: catalogoPublico(),
      maximos: { bloques_por_pagina: MAX_BLOQUES_POR_PAGINA, paginas: MAX_PAGINAS },
      nota: 'Un bloque es { type, data }. Los de categoría "sistema" no llevan datos: sacan su contenido del evento.',
    };
  },

  async ver_landing(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const { data, error } = await supabase
        .from('eventos').select('titulo, slug, paginas').eq('id', ev.id).single();
      if (error) return { error: error.message };
      ...
      return {
        ...
        url_publica: `/explorar/${data.slug}`,
        paginas,
        total_bloques: paginas.reduce((n, p) => n + (Array.isArray(p?.blocks) ? p.blocks.length : 0), 0),
        nota: paginas.length ? undefined : 'Todavía no hay landing: guardar_landing la crea.',
      };
    } catch (e) { return { error: e.message }; }
  },

  async guardar_landing(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const paginas = Array.isArray(input.paginas) ? input.paginas : null;
      if (!paginas) return { error: 'Falta `paginas`, y tiene que ser una lista.' };

      /* Se valida ANTES de tocar la base: media landing guardada es peor que
         ninguna, porque nadie sabe qué mitad quedó. */
      const fallo = fallaPaginas(paginas);
      if (fallo) return { error: fallo, nota: 'No se guardó nada. Mira catalogo_bloques_landing.' };

      /* Cada bloque necesita un id estable: es lo que usa el editor para
         reordenar y lo que apunta un embed exportado «de esta sección exacta».
         Se respeta el que venga y se inventa sólo el que falte. */
      const conIds = paginas.map(p => ({
        ...p,
        blocks: (Array.isArray(p?.blocks) ? p.blocks : []).map(b => ({
          ...b,
          id: b.id || `b_${Math.random().toString(36).slice(2, 10)}`,
        })),
      }));

      const { error } = await supabase.from('eventos').update({ paginas: conIds }).eq('id', ev.id);
      if (error) return { error: error.message };
      return {
        ok: true,
        paginas: conIds.length,
        bloques: conIds.reduce((n, p) => n + p.blocks.length, 0),
        nota: 'La landing quedó reemplazada por lo que mandaste.',
      };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async listar_codigos_descuento(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      /* Los del panel y los del chat son ya los mismos: una sola lista, que es
         lo que hace que «¿qué códigos tengo?» tenga una sola respuesta. */
      const { data, error } = await supabase
        .from('promociones')
        .select('codigo, tipo, valor, limite_usos, usos, activo, vigente_hasta, ticket_id, min_cantidad')
        .eq('evento_id', ev.id)
        .order('created_at', { ascending: false });
      if (error) return { error: error.message };
  ...
  async listar_expositores_networking(userId, input) {
    try {
      ...
      const { data: expos } = await supabase
        .from('networking_expositores').select(COLS_TARJETA).eq('evento_id', ev.id).order('nombre', { ascending: true });
      ...
      /* Va el NOMBRE de la zona, no su id: quien lee esto es un modelo que
         tiene que poder contestar "el stand A-12 está en la Zona Gamer". Un
         identificador no le sirve de nada y encima lo repetiría en voz alta. */
      const zonas = await zonasDelEvento(ev.id).catch(() => []);
      const conZonaResuelta = conZona(expos || [], zonas);
      return {
        evento: ev.titulo,
        total: conZonaResuelta.length,
        expositores: conZonaResuelta.map(e => ({
          nombre: e.nombre, stand: e.stand, zona: e.zona_nombre,
          horarios_publicados: conteo[e.id] || 0,
        })),
      };
    } catch (e) { return { error: e.message }; }
  },
  ...
  async listar_equipos_torneo(userId, input) {
    try {
      ...
      const { data: equipos, error: eEquipos } = await supabase.from('torneo_equipos').select('nombre, grupo').eq('torneo_id', torneo.id).order('created_at', { ascending: true });
      /* Se lo contesta a una IA que va a decírselo a alguien con esas palabras:
         «total: 0» lo va a leer en voz alta como «no hay equipos inscritos». */
      if (eEquipos) return { error: `No pude leer los equipos del torneo: ${eEquipos.message}` };
...
async function ejecutarTool(userId, name, input, acciones, via) {
  ...
  /* Rastro de lo que el agente cambió.
   *
   * Medido antes de esto: 52 escrituras a la base desde aquí y CERO auditadas.
   * El panel anota cada cambio de estado, cada cortesía, cada miembro que sale
   * del equipo; por el agente no se anotaba nada. Y desde que un token del MCP
   * se puede conceder con permiso de «Dinero y accesos», eso deja de ser
   * teórico.
   *
   * Va aquí y no en cada herramienta porque son 68: una llamada por
   * herramienta son 68 sitios donde olvidarla, y la 69ª nacería sin ella.
   *
   * Sin `await`: la anotación no puede retrasar la respuesta ni tumbarla. Si
   * falla, la acción ya ocurrió. */
  anotarAccionDelAgente({ userId, nombre: name, input, resultado: result, via })
    .catch(() => {});
