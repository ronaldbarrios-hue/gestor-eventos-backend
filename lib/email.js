  ...
  for (const [id, sufijo] of [['cpanel', ''], ['smtp2', '2']]) {
    ...
    try {
      ...
      const transport = nodemailer.createTransport({
        ...
        auth: { user, pass },
        /* family: 4 — fuerza IPv4. Render (y otros hostings) resuelven
           smtp.hostinger.com también por IPv6, y esa ruta no es alcanzable
           desde su red: sin esto, cada envío fallaba con
           «ESOCKET connect ENETUNREACH» o «ETIMEDOUT» contra una IP v6,
           nunca llegaba a intentar la v4 que sí funciona. */
        family: 4,
      });
...
function transporteDeEvento(smtp) {
  ...
  const t = nodemailer.createTransport({
    ...
    family: 4,           // ver la nota junto al buzón de plataforma más arriba
  });
