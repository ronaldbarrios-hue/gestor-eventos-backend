/* GESTEK — Envío de email transaccional desde el backend.

   Provider-agnóstico:
   - Si hay GMAIL_USER + GMAIL_APP_PASSWORD → Gmail SMTP (nodemailer).
   - Si no, y hay RESEND_API_KEY → Resend (fetch, sin dependencia).
   - Si no hay nada configurado → no-op silencioso (no rompe el flujo).

   Best-effort: nunca lanza. */

let _transport = null;
let _mode = null; // 'gmail' | 'resend' | null
...
function init() {
  ...
  if (gUser && gPass) {
    try {
      ...
      _mode = 'gmail';
      console.log('[email] modo Gmail activado para:', gUser);
      return;
    } catch (e) {
  ...
  _mode = null;
  console.warn('[email] sin proveedor configurado (GMAIL_* o RESEND_API_KEY). Emails deshabilitados.');
}

function fromAddress() {
  return process.env.EMAIL_FROM
    || (process.env.GMAIL_USER ? `GESTEK <${process.env.GMAIL_USER}>` : 'GESTEK <onboarding@resend.dev>');
}

/**
 * sendMail({ to, subject, html })  — to: string | string[]
...
async function sendMail({ to, subject, html }) {
  ...
  try {
    if (_mode === 'gmail') {
      console.log('[email] intentando enviar a:', destinatarios, 'desde:', fromAddress());
      const info = await _transport.sendMail({ from: fromAddress(), to: destinatarios.join(','), subject, html });
      console.log('[email] enviado OK:', info.messageId, info.response);
      return { ok: true };
    }
    /* resend */
    ...
    console.error('[email] ERROR COMPLETO:', e.code, e.message, e.responseCode, e.response);
    return { ok: false, error: e.message };
  }
}

/* Plantilla HTML para notificación de tarea (mismo estilo dark que recordatorios) */
