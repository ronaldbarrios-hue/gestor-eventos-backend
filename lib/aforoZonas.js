...
async function agendaPorZona(eventoId, zonas, ahoraISO = null) {
  ...
  const { data: sesiones } = await supabase
    ...
    /* `descripcion` viaja desde el 11-sep: el mapa publico enseñaba de cada
       actividad la hora y el titulo, y «Torneo gamer» a secas no dice si hay
       que llevar equipo, si se juega por parejas ni a quien va dirigido. La
       columna existe y 14 de las 18 actividades de produccion la tienen
       rellena, asi que el dato estaba ahi sin llegar a la pantalla. */
    .select(`id, titulo, descripcion, tipo, inicio, fin, track, ubicacion, zona_id, cupo, inscritos, requiere_inscripcion,
  ...
  for (const z of zonas) {
    ...
    const conEstado = suyas.map(s => {
      ...
      const estado = !ini ? 'sin_hora'
        ...
        : 'ahora';
      return {
        id: s.id, titulo: s.titulo, descripcion: s.descripcion || null,
        tipo: s.tipo, inicio: s.inicio, fin: s.fin,
