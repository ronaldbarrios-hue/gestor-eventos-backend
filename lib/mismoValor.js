 ...
 *
 * Y una lista o un objeto SIN NADA DENTRO también. Medido: hoy ninguno de los
 * 15 eventos tiene `gallery`, `links` o `branding` en null —todos guardan `[]`
 * o `{}`— así que esto no arregla nada que esté roto ahora. Cierra el caso de
 * un evento nuevo, o de una columna futura, donde la base tenga null y el
 * formulario mande `[]`: sin esto, eso sería un «cambió» de mentira y un 403
 * donde hoy se guarda bien.
 *
 * No esconde ningún cambio real: vaciar una galería que tenía fotos sigue
 * siendo un cambio, porque `['a.jpg']` no está vacía. */
const vacio = (v) => {
