...
const EXECUTORS = {
  ...
  async crear_torneo(userId, input) {
    try {
      ...
      const { data: existente } = await supabase.from('torneos').select('id').eq('evento_id', input.evento_id).maybeSingle();
  ...
  async listar_equipos_torneo(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const { data: torneo } = await supabase.from('torneos').select('id, nombre, formato, estado').eq('evento_id', ev.id).maybeSingle();
  ...
  async agregar_equipo_torneo(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const { data: torneo } = await supabase.from('torneos').select('id, estado').eq('evento_id', ev.id).maybeSingle();
  ...
  async generar_fixture_torneo(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const { data: torneo } = await supabase.from('torneos').select('*').eq('evento_id', ev.id).maybeSingle();
      if (!torneo) return { error: 'Este evento no tiene torneo configurado.' };
  ...
  async ver_partidos_torneo(userId, input) {
    try {
      const ev = await assertOwn(input.evento_id, userId);
      const { data: torneo } = await supabase.from('torneos').select('id').eq('evento_id', ev.id).maybeSingle();
      if (!torneo) return { error: 'Este evento no tiene torneo configurado.' };
  ...
  async _buscarPartidoTorneo(eventoId, equipoANombre, equipoBNombre) {
    const { data: torneo } = await supabase.from('torneos').select('id').eq('evento_id', eventoId).maybeSingle();
    if (!torneo) return { error: 'Este evento no tiene torneo configurado.' };
    const { data: equipos } = await supabase.from('torneo_equipos').select('id, nombre').eq('torneo_id', torneo.id);
