...
function bloqueCurvo({ cx = 0, cy = 0, rInterior = 200, rExterior = 320,
  ...
  /* Dónde va la etiqueta del bloque.
   *
   * No es el centro de su caja: en un arco ancho —una general que abarca 190°—
   * el centro de la caja cae en el AGUJERO, fuera de la figura, y el nombre se
   * dibuja flotando en la pista. Se vio en la primera plantilla: «General A»
   * simplemente no aparecía.
   *
   * Aquí sí se sabe la respuesta buena, porque se conocen el ángulo y el radio:
   * el punto medio del arco, a media altura entre el borde interior y el
   * exterior. Siempre cae dentro. */
  const medio = ((eje + (desde + hasta) / 2) * Math.PI) / 180;
  const rMedio = (Number(rInterior) + Number(rExterior)) / 2;

  /* Ida por dentro y vuelta por fuera: así el polígono se cierra sin cruzarse.
     Al revés sale un lazo, que se dibuja como un nudo. */
  return {
    puntos: [...arco(rInterior, desde, hasta), ...arco(rExterior, hasta, desde)],
    centro: [redondear(cx + Math.cos(medio) * rMedio), redondear(cy + Math.sin(medio) * rMedio)],
  };
}
