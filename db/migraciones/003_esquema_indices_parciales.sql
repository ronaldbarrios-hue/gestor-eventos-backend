...
/* WHERE padre_id IS NOT NULL — y padre_id está en la clave. El lower(nombre)
   del original lo hace la colación de la tabla (utf8mb4_0900_as_ci compara
   sin distinguir mayúsculas, igual que lower(), y sí distingue acentos).
   Si alguien cambia la colación a una _bin, esta unicidad se afloja en
   silencio: es el único punto del esquema donde eso importa. */
CREATE UNIQUE INDEX `torneo_categorias_unica_hija`
  ON `torneo_categorias` (`evento_id`, `padre_id`, `nombre`);
