 ...
 * `(Number(page) - 1) * Number(limit)` sin red — que con `page=abc` da `NaN`.
 *
 * Medido contra producción antes de escribir esto: `?page=abc` responde **200
 * con la lista vacía**. No es una caída, es peor — para una lista, «vacío» es
 * una respuesta creíble. La página de explorar decía «no hay eventos» habiendo
 * siete, y el `total: 7` que iba al lado no lo miraba nadie.
 *
 ...
 *
 * Nunca devuelve `NaN` ni negativos: eso es el punto — un `NaN` no da error,
 * da una lista vacía que parece un dato. `porDefecto` y `tope`
