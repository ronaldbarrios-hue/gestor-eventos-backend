...
function plantillaTicket({ eventoTitulo, eventoCoverUrl, eventoFecha, eventoLugar, nombre, codigo, qrToken, tipoNombre, linkTicket, gratis }) {
  ...
  const qrImgUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=12&data=${encodeURIComponent(qrValue)}`;
      ...
      <tr>
        <td align="center">
          <div style="display:inline-block;background:#ffffff;padding:14px;border-radius:16px;">
            <img src="${qrImgUrl}" width="300" height="300" alt="Código QR de tu entrada" style="display:block;width:300px;height:300px;" />
          </div>
        </td>
      </tr>
    </table>
...
/* ────────────────────────────────────────────────────────────
   Plantilla: recordatorio de evento próximo (T-7d / T-1d / T-1h) */
function plantillaRecordatorioEvento({ nombre, tipo, eventoTitulo, eventoCoverUrl, eventoFecha, eventoLugar, codigo, linkEvento }) {
  const fecha = fmtFecha(eventoFecha);
  const TITULOS = {
    t7d: 'Tu evento es en 7 días',
    t1d: 'Tu evento es mañana',
    t1h: 'Tu evento empieza en 1 hora',
  };
  const KICKERS = {
    t7d: 'Recordatorio · 1 semana antes',
    t1d: 'Recordatorio · mañana',
    t1h: 'Recordatorio · muy pronto',
  };

  const body = `
    <h1 style="margin:0 0 14px;font-size:22px;font-weight:700;color:#F8FAFC;line-height:1.3;">
      ${TITULOS[tipo] || 'Recordatorio de tu evento'}${nombre ? `, ${escapeHtml(nombre.split(' ')[0])}` : ''}
    </h1>
    <p style="margin:0 0 22px;font-size:15px;line-height:1.6;color:#94A3B8;">
      No te olvides — tu entrada ya está confirmada. Aquí el resumen:
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
      style="background:#0D1525;border:1px solid #1e293b;border-radius:16px;margin-bottom:22px;">
      <tr><td style="padding:20px;">
        ...
        <p style="margin:0 0 14px;font-size:14px;color:#E2E8F0;">${escapeHtml(eventoLugar)}</p>` : ''}
        <p style="margin:0 0 3px;font-size:11px;color:#64748B;text-transform:uppercase;letter-spacing:.08em;">Código de tu entrada</p>
        <p style="margin:0;font-size:20px;font-weight:800;color:#F8FAFC;letter-spacing:.06em;font-family:monospace;">${escapeHtml(codigo)}</p>
      </td></tr>
    </table>`;

  return emailLayout({
    kicker: KICKERS[tipo] || 'Recordatorio',
    coverUrl: eventoCoverUrl,
    eventoTitulo,
    bodyHtml: body,
    ctaText: 'Ver mi entrada',
...
module.exports = { sendMail, plantillaTarea, plantillaTicket, plantillaInvitacionEquipo, plantillaRecordatorioEvento };
